...
const About = () => {
  ...
  useEffect(() => {
    if (!aboutConfig.headline) return;
  ...
  return (
    ...
    >
      ...
      <div ref={textRef} className="max-w-6xl mx-auto pt-32 pb-20 px-8 lg:px-16">
        <p className="reveal-text text-[#8c8c91] text-sm tracking-widest uppercase mb-6" style={{ willChange: 'transform, opacity' }}>
          {aboutConfig.label}
        </p>
        <h2 className="reveal-text text-[#f0f0f0] text-4xl md:text-5xl lg:text-6xl font-light tracking-tight mb-8" style={{ willChange: 'transform, opacity' }}>
          {aboutConfig.headline}
        </h2>
        <p className="reveal-text text-[#8c8c91] text-lg md:text-xl max-w-2xl leading-relaxed" style={{ willChange: 'transform, opacity' }}>
          ...
          <div className="grid grid-cols-3 gap-4 lg:gap-5">
            ...
            <div className="gallery-col space-y-4 lg:space-y-5 will-change-transform" data-speed="-80">
              {col1Images.map((img, i) => (
                <div key={i} className="gallery-img-wrap overflow-hidden rounded-lg will-change-transform" data-offset={i === 0 ? "60" : "120"}>
                  <img src={img.src} alt={img.alt} className="w-full h-auto object-cover" style={{ aspectRatio: i === 0 ? '3/4' : '4/5' }} />
                  <p className="text-[#8c8c91]/50 mt-3 text-xs">{img.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="gallery-col space-y-4 lg:space-y-5 pt-20 lg:pt-32 will-change-transform" data-speed="100">
              {col2Images.map((img, i) => (
                <div key={i} className="gallery-img-wrap overflow-hidden rounded-lg will-change-transform" data-offset={i === 0 ? "80" : "160"}>
            ...
            <div className="gallery-col space-y-4 lg:space-y-5 will-change-transform" data-speed="-120">
              {col3Images.map((img, i) => (
                <div key={i} className="gallery-img-wrap overflow-hidden rounded-lg will-change-transform" data-offset={i === 0 ? "40" : "140"}>
      ...
      {aboutConfig.stats.length > 0 && (
        <div ref={statsRef} className="relative z-10 max-w-7xl mx-auto px-8 lg:px-16 py-24 bg-[#1a1a1a]">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-[#8c8c91]/10 pt-12">
            {aboutConfig.stats.map((stat, i) => (
              <div key={i} className="stat-item">
                <p className="text-[#f0f0f0] text-4xl md:text-5xl font-light mb-2">{stat.value}</p>
                <p className="text-[#8c8c91] text-sm">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {aboutConfig.bottomText && (
        <div className="relative z-10 max-w-7xl mx-auto px-8 lg:px-16 pb-32 bg-[#1a1a1a]">
          <div className="grid grid-cols-12">
            <div className="col-span-12 md:col-span-5 md:col-start-8">
              <p className="reveal-text text-[#8c8c91] text-base lg:text-lg leading-relaxed">
                {aboutConfig.bottomText}
              </p>
            </div>
          </div>
